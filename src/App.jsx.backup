...
export default function App() {
  ...
  // 登入處理
  const handleLogin=()=>{
    const acc=accounts.find(a=>a.email===loginEmail&&a.password===loginPwd);
    if(!acc){setLoginErr("帳號或密碼錯誤");return;}
    setCurrentRole(acc.role);
    setCurrentUser(acc.name);
    setLoginErr("");
    if(acc.firstLogin){setMustChangePwd(true);}
  ...
  if(!isLoggedIn) return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1e1b4b,#4338ca)",
      ...
      <div style={{background:"#fff",borderRadius:18,padding:"40px 36px",width:400,boxShadow:"0 24px 64px rgba(0,0,0,.25)"}}>
        ...
        {!forgotMode
          ...
          :<div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {forgotSent
              ...
              :<button onClick={()=>{
                  if(accounts.find(a=>a.email===forgotEmail)) setForgotSent(true);
                  else setLoginErr("找不到此Email帳號");
                }}
                style={{background:"#6366f1",color:"#fff",border:"none",borderRadius:9,padding:"12px",
        ...
        <div style={{marginTop:20,padding:"12px",background:"#fffbeb",borderRadius:8,
          border:"1px solid #fde68a",fontSize:11,color:"#92400e",lineHeight:1.6}}>
          ⚠️ <strong>安全提醒：</strong>此系統為前端模擬登入，密碼儲存於瀏覽器記憶體中。
          如需正式環境請整合後端服務（Firebase / Supabase）以確保安全。
        </div>
      </div>
    </div>
  );
    ...
    <div style={{fontFamily:"'Noto Sans TC','DM Sans',sans-serif",background:"#f1f5f9",minHeight:"100vh",display:"flex"}}
      ...
      <div style={{width:210,background:"#0f172a",flexShrink:0,display:"flex",flexDirection:"column",
        ...
        <div style={{padding:"14px 20px",borderTop:"1px solid #1e293b"}}>
          ...
          <button onClick={()=>{setIsLoggedIn(false);setLoginEmail("");setLoginPwd("");}}
